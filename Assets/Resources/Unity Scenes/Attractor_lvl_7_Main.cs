...
public class Attractor_lvl_7_Main : MonoBehaviour
{
    ...
    void Start()
    {
        for (int i = 0; i < Hub.Instance.mStageInfoList.Count; i++)
        {
            ...
            if (Hub.Instance.mStageInfoList[i].mSceneName == "Attractor_lvl_7")
    ...
    void Update()
    {
        ...
        {
            mEnemy.transform.position = new Vector3(Mathf.Sin(Time.time * 3) * 6, Mathf.Cos(Time.time * 2) * 2, 4.0f);
        }
