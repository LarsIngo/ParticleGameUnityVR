...
public class Attractor_lvl_4_Main : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < Hub.Instance.mStageInfoList.Count; i++)
        {
            ...
            if (Hub.Instance.mStageInfoList[i].mSceneName == "Attractor_lvl_4")
    ...
    void Update()
    {
        ...
        enemies.transform.position = new Vector3(Mathf.Sin(Time.time) * 1.5f, Mathf.Cos(Time.time) * 1.5f, 0.0f);
    ...
    { 
        ...
        {
            GameObject enemy = Factory.CreateBasicEnemy(Vector3.forward * 3 + Vector3.right * 3 * Mathf.Sin(fac * i) + Vector3.up * 3 * Mathf.Cos(fac * i), 1000);
